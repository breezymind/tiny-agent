...
pi 코딩 에이전트의 반자동 계획→구현→검증 파이프라인 확장. 그릴 인터뷰로 계획을 확정하고, 자식 pi 프로세스로 구현·테스트·검수·문서 정합화를 실행한다. 구현 에이전트는 이 파일과 최신 아키텍처 ADR을 먼저 읽어야 한다.
...
**아키텍처 기준 ADR**:
현재 구현의 책임 경계, source of truth, 파이프라인 불변조건을 기록한 `docs/adr/0003-architecture-baseline-and-read-gate.md`. 벡터 검색 결과가 아니라 구현 전에 직접 읽어야 하는 기준 문서다.
_Avoid_: ADR 요약만 읽기, 검색 결과를 결정으로 취급하기

**아키텍처 읽기 게이트**:
계획·직접 코딩·병렬 코딩·개선 에이전트가 `CONTEXT.md`, 아키텍처 기준 ADR, 관련 ADR을 먼저 읽고 적용할 source of truth와 책임 경계를 확인하는 공통 절차.
_Avoid_: 태스크 본문만 읽고 구현하기, semantic search만으로 구현하기

모든 목표 체크리스트에는 변경된 코드가 정의된 아키텍처의 책임 경계·source of truth·불변조건을 지키는지 확인하는 항목을 작업 크기와 무관하게 포함한다.

**문서 벡터 인덱스**:
`CONTEXT.md`, `docs/*.md`, `docs/adr/**/*.md`를 section 단위로 sqlite-vector에 저장해 관련 문서를 찾는 보조 인덱스. 문서 직접 읽기를 대체하지 않으며, 원문 파일이 기준이다. `docs/*.md`는 프로젝트 운영·구조 문서(`doc`)로, ADR은 결정 문서(`adr`)로 분류한다.
_Avoid_: 문서 DB를 새로운 source of truth로 취급하기, 모든 Markdown 자동 복사하기
