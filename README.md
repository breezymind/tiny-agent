...
Pi 전역 런타임은 `~/.pi/agent/mcp.json`에서 CodeGraph MCP를 `keep-alive`로
기본 로드합니다. 따라서 어느 프로젝트에서 시작해도 `codegraph_explore`를 바로
사용할 수 있으며, 프로젝트별 `.mcp.json`에서 필요하면 재정의할 수 있습니다.
...
L2/L3 업무 진입 시에는 위 파이프라인에 앞서 현재 목표를 SQLite issue-store와
SQLite 아키텍처 문서의 semantic search로 반드시 검색합니다(`--limit 3`). 검색 결과는 계획·직접 구현 프롬프트의
참고 문맥으로 주입되며, 검색 명령이나 임베딩 환경이 실패하면 파이프라인을 시작하지
않습니다. 관련 결과가 0건인 것은 정상적인 검색 결과로 허용합니다.

L0/L1은 의도적인 예외입니다. 작은 단일 작업(L0)은 인터뷰·PRD·이슈 분해와
semantic search를 생략하고 최소 구현·검증 경로로 바로 진입합니다. 집중 계획(L1)은
grilling과 `grill-checklist`를 유지하되 전체 문서/이슈 파이프라인과 semantic search를
생략합니다. 두 경로 모두 체크리스트와 필요한 실제 검증은 유지합니다.
독립 검수는 L0/L1에서 항상 생략하고, L2/L3에서만 `autoReview` 설정에 따라 수행합니다.
L1/L2/L3의 체크리스트는 grilling 질문 턴, 실제 사용자 응답, 완료 marker가 모두
확인될 때까지 실행되지 않습니다. L0 direct 경로만 이 게이트의 예외입니다.

병렬 코딩 태스크는 선택·실행 모두 기본 최대 2개의 활성 코드 에이전트로 제한합니다.
동시 읽기 부하를 더 낮추려면 `PI_LOOP_AGENT_MAX_PARALLEL_CODING_READERS`에
1 이상의 값을 지정할 수 있습니다.
Git 저장소가 clean하면 태스크마다 임시 detached worktree를 사용해 작업 디렉터리를
서로 격리합니다. 현재 미커밋 변경이나 Git 저장소가 없는 경우에는 해당 변경과
로컬 런타임 파일을 보존하기 위해 기존 snapshot 방식으로 자동 fallback합니다.
