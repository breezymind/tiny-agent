...
자동 계획 진입 시에는 위 파이프라인에 앞서 현재 목표를 issue-store의 semantic search로
반드시 검색합니다(`--limit 8`). 검색 결과는 계획 프롬프트의 참고 문맥으로 주입되며, 검색
명령이나 임베딩 환경이 실패하면 첫 메시지라도 일반 대화로 우회하지 않고 파이프라인을
중단합니다. 관련 결과가 0건인 것은 정상적인 검색 결과로 허용합니다.
...
기존 `docs/tasks/*.md`는 최초 마이그레이션 입력과 감사 자료로만 사용하며, 이후 SQLite를 직접 사용합니다.
Pi를 기존 프로젝트에서 시작했을 때 `docs/issues.sqlite`가 없고 레거시 `T-###` 문서가 발견되면,
loop-agent가 DB를 만들고 Markdown 이슈를 자동으로 옮기며, 원본 문서는 보존됩니다.
...
`CONTEXT.md`, `docs/*.md`, `docs/adr/**/*.md`는 `index-architecture`로 section 단위 색인되어
`search-architecture`로 관련 문서 후보를 찾을 수 있습니다. 이 벡터 검색은 보조 수단이며,
계획·직접 코딩·병렬 코딩·개선 에이전트는 구현 전에 `CONTEXT.md`와 아키텍처 기준 ADR을
`read` 도구로 직접 읽어야 합니다. 검색 결과만으로 아키텍처 결정을 확정하지 않습니다.
`docs/issues.sqlite`가 없으면 loop-agent가 세션 시작과 직접/계획 작업 준비 단계에서 이 색인을 자동 생성·갱신합니다. `docs/*.md`는 `doc` 유형으로, ADR은 `adr` 유형으로 저장됩니다.
...
`prune-migrated --yes`는 모든 Markdown `T-###`가 DB에 존재할 때만 기존 이슈 본문을 source-of-truth 안내문으로 축약합니다.
