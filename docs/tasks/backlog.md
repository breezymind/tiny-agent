...
4. loop-agent 사용자로서, 나는 문서 정합화가 자동(/goal)이든 수동(/loop-agent review)이든 검수 PASS되면 항상 실행되기를 원한다, 그래야 진입 경로와 무관하게 문서가 최신으로 유지된다.
...
8. loop-agent 사용자로서, 나는 워크플로를 clear하거나 새 /goal을 시작하면 진행 중이던 정합화 자식도 함께 회수되기를 원한다, 그래야 보이지 않는 부하가 남지 않는다.
...
- **트리거 지점:** 검수 결과 overall==PASS인 모든 경로(자동 /goal, 수동 /loop-agent review)에서 실행. PASS 분기 한 곳에만 두어 불변식이 코드상 한 지점에서 성립하게 한다.
...
`runExecutionReviewLoop`의 검수 overall==PASS 분기에서, 기존 종료 처리(reviewed로 귀결) 직전에 정합화 단계를 배선한다. 자동(/goal)·수동(/loop-agent review) 무관하게 PASS되는 모든 경로가 이 분기를 지나므로 한 곳만 수정한다. 진입 전 reviewStage를 "reconciling"으로 전이하고(ReviewStage 타입에 추가) persist, 정합화 자식을 실행해 결과를 `loop-agent-doc-reconciliation` customType으로 appendEntry+sendMessage(display)한다. 전체를 try/catch로 감싸 예외/타임아웃 시 warning만 남기고, 성공/실패 무관 마지막에 reviewStage="reviewed", autoMode=false, workflowId=null로 귀결한다.
...
정합화 자식을 await한 뒤 `isCurrentWorkflow(workflowId)`를 검사해, 그 사이 사용자가 /loop-agent clear하거나 새 /goal을 시작해 워크플로가 교체되었으면 정합화 결과의 appendEntry·sendMessage·persist를 수행하지 않고 조용히 반환한다. 다른 단계(runCodingAgent/runTestAgent/runValidatedReview 뒤)의 isCurrentWorkflow 가드와 대칭이다.
